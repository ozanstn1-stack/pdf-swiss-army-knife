...
- **Model selection:** the default is **`deepseek-flash`** (DeepSeek V4.1 Flash, 1M context, the current canonical name; `deepseek-v4-flash` is still accepted and routed to the same model). Settings offers `deepseek-flash`, the legacy alias, `deepseek-v4-flash-vision-exp`, `deepseek-v4-pro` and the legacy `deepseek-chat` / `deepseek-reasoner` aliases, plus a free-text field for any other model id.
- **Limits are configurable:** *Context budget (input tokens)* goes up to the full **1,000,000-token** window (presets 64K…1M) and controls how much document text is sent per request — at 1M a ~2M-character document is summarised in a single request. *Max output tokens* goes up to the API ceiling of **384,000** (presets 4K…384K). Both are clamped to the documented API limits.
